...
void BallRenderer::renderBalls(Falcon::SpriteBatch& spriteBatch, const std::vector<Ball>& balls,
                               const glm::mat4& projectionMatrix) 
{
    ...
    {
        m_program = std::make_unique<Falcon::ShaderProgram>();
        m_program->compileShaders("src/Shaders/colorShading.vert", "src/Shaders/colorShading.frag");
...
void MomentumBallRenderer::renderBalls(Falcon::SpriteBatch& spriteBatch, const std::vector<Ball>& balls,
                                       const glm::mat4& projectionMatrix) 
{
    ...
    // Make sure the shader uses texture 0
    glActiveTexture(GL_TEXTURE0);
    GLint textureUniform = m_program->getUniformLocation("sampler");
    glUniform1i(textureUniform, 0);
