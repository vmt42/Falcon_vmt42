...
public:
    ...
    void update(bool isMouseDown,const glm::vec2 position, const glm::vec2 direction,
                std::vector<Slash> &slashes, float deltaTime, float rotate);

private:
    void attack(const glm::vec2 position, const glm::vec2 direction, std::vector<Slash> &slashes, float rotate);
